...
tf.app.flags.DEFINE_string('data_name', 'data.csv', 'train data name')
tf.app.flags.DEFINE_string('model_name', '', 'model name')
tf.app.flags.DEFINE_string('model_dir', './checkpoint', 'model dir')
tf.app.flags.DEFINE_string('model_path', '', 'model path')
tf.app.flags.DEFINE_string('model_dict_path', '', 'model dict path')
...
mkdir -p /model/boc-web-predict/ckpt_198d_8f_322;
mkdir -p /model/boc-web-predict/ckpt_198d_8f_322_dict;
python /src/boc-web-predict/transaction_train_v3_k8s.py \
       --data_dir=/data/boc-web-predict \
       --data_name=res_198days_8feature_0827_0312_fixed.csv \
       --model_name=ckpt_198d_8f_322 \
       --model_dir=/model/boc-web-predict \
       --model_path=/model/boc-web-predict/ckpt_198d_8f_322 \
       --model_dict_path=/model/boc-web-predict/ckpt_198d_8f_322_dict \
       --log_path=/log/boc-web-predict \
       1>>/log/boc-web-predict/boc-web-predict.log 2>>/log/boc-web-predict/boc-web-predict.errlog ;"]
...
model_dict_path = FLAGS.model_dict_path

train_data_path = data_dir + '/' + data_name

print('model name: %s.' % model_name)
print('model saving path: %s.' % model_path)
print('dict saving path: %s.' % model_dict_path)
print('training data path: %s.' % train_data_path)
print('training log path: %s.' % log_path)
...
if os.path.isdir(model_dict_path) is False:
    os.mkdir(model_dict_path)
normalized_count_data_dir = model_dict_path + '/normalized_count.dict'
...
'''
normalized_features_mean_data_dir = model_dict_path + '/normalized_features_mean.npy'
normalized_features_std_data_dir = model_dict_path + '/normalized_features_std.npy'
...
def train_lstm():
    ...
    if continue_training == 1:
        module_file = tf.train.latest_checkpoint(model_path + '/')  # restore
        ...
        for epoch_step in range(max_epoch):
            ...
            while end < len(train_x):
                ...
                if step % print_step == 0 or loss_ < min_loss:
                    ...
                    print("Saving model: ", saver.save(sess, model_path + '/tp_model.ckpt'))
                    min_loss = loss_
                step += 1
